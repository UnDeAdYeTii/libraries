...
class Debug
{
    ...
    public static function dump($var, $die = true)
    {
        ...
        self::init_style();
        print '<pre>';
        $output = self::$method == 'print_r' ? $output = print_r(self::debug_makevisible($var), true) : json_encode($var,
            JSON_PRETTY_PRINT);
    ...
    private static function debug_makevisible($variable, $name = 'main')
    {
        ...
        if (is_array($variable)) {
            ...
            if ($variable === null) {
                return '<i class="pu">null</i>';
            }
            if ($variable === false) {
                return '<i class="pu">false</i>';
            }
            if ($variable === true) {
                return '<i class="pu">true</i>';
            }
            if ($variable === '') {
                return '<i class="pu">empty-string</i>';
            }
            if (is_string($variable)) {
                return '<i class="y">' . $variable . '</i>';
            }
        }
